...
def get_compartments(rugsacks):
    compartments = [[ items[:len(items)//2], items[len(items)//2:] ] for items in rugsacks]
...
def test_solution():
    ...
    common_items = [find_common_items(compartment_1, compartment_2) for compartment_1, compartment_2 in test_compartments]
...
test_rugsacks = ["vJrwpWtwJgWrhcsFMMfFFhFp", "jqHRNqRjqzjGDLGLrsFMfFZSrLrFZsSL", "PmmdzqPrVvPwwTWBwg", "wMqvLMZHhHMvwLHjbvcjnnSBnvTQFn", "ttgJtRGJQctTZtZT", "CrZsJsPPZsGzwwsLwLmpwMDw"]
test_solution()
...
def solution_1(rugsacks):
    ...
    common_items = [find_common_items(compartment_1, compartment_2) for compartment_1, compartment_2 in compartments]
    common_items_values = [get_item_value(item) for item in common_items]
    items_sum = sum(common_items_values)
